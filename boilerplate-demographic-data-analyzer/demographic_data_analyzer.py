...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round((data[higher_education]['salary'] == '>50K').mean() * 100, 1)
    lower_education_rich = round((data[lower_education]['salary'] == '>50K').mean() * 100, 1)
    ...
    for country, df in data.groupby('native-country'):
        percentage = (df['salary'] == '>50K').sum() / df['salary'].count()
    ...
    top_IN_occupation = data[(data['native-country'] == 'India') & (data['salary'] == '>50K')]['occupation'].mode()[0]
